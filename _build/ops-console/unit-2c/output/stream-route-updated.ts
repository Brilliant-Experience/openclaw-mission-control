 ...
 *
 * Session routing: when `sessionKey` is provided the request is forwarded to
 * the gateway with the `x-openclaw-session-key` header, allowing the gateway
 * to route the message to a specific agent session (e.g. for the Scotty tab
 * in the OpsConsole).  When absent the header is omitted and existing default
 * behaviour is fully preserved — this is a backwards-compatible addition only.
 */
...
interface StreamRequest {
  ...
  /** Optional agent identifier — falls back to "main". */
  agentId?: string;
  /** Deprecated alias for agentId; kept for backwards compatibility. */
  agent?: string;
  /** Optional model override. */
  model?: string;
  /**
   * Optional target session key.  When provided the gateway routes the
   * message to the specified agent session rather than the default session.
   */
  sessionKey?: string;
}
...
export async function POST(req: Request): Promise<Response> {
  ...
  try {
    ...
    // Extract and validate sessionKey — must be a non-empty string.
    ...
    // Session routing — only set the header when a key was explicitly provided.
    // Absence of the header preserves the gateway's existing default routing.
    if (sessionKey) {
    ...
    if (!gwRes.ok) {
      ...
      logRequest("/api/chat/stream", gwRes.status, Date.now() - start, {
        agentId,
    ...
    logRequest("/api/chat/stream", 200, Date.now() - start, {
      agentId,
      model: model ?? `openclaw:${agentId}`,
